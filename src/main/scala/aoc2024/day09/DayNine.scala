...
object DayNine extends DailyChallenge[Long]:
  ...
  private def defragmentBlocks(drive: Drive): Drive =
    @tailrec
    def loop(idx: Int = drive.size - 1, defragmented: Drive = drive): Seq[MemoryBlock] =
      defragmented.lift(idx) match
        case Some(file: File) =>
          defragmented.slice(0, idx).indexOfSlice((0 until file.length).map(_ => Free)) match
  ...
  @tailrec
  private def defragment(fs: FileSystemContent): FileSystemContent =
    fs.indexOf(-1) match
      case -1 => fs
      case nextFreeSpace =>
        fs.lastOption match
  ...
  private sealed trait MemoryBlock
