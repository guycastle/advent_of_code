...
object DayFour extends DailyChallenge[Int]:
  ...
  override lazy val day: LocalDate = LocalDate.of(2023, 12, 4)
  ...
  @tailrec
  private def evaluateScratchCards(
      reversedCards: Seq[ScratchCardResult],
      cardMap: Map[Int, ScratchCardResult] = Map.empty): Seq[ScratchCardResult] = reversedCards.headOption match
  ...
  override def partTwo(input: Seq[String]): Int = evaluateScratchCards(input.flatMap(_.toScratchCardResult).reverse)
    .foldLeft(0)((sum, card) => sum + card.cardsWonTotal)
